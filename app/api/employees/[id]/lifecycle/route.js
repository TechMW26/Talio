...
import { applyLifecycleAction, getLifecycleProgress, hydrateEmployeeLifecycle } from '@/lib/hrms/employeeLifecycle.server'
...
async function authorize(request, id) {
  ...
  const auth = await getAuthAndModels(request, ['Employee', 'HrmsWorkflow', 'HrmsWorkflowEvent'])
  if (!auth.success) {
...
export async function GET(request, { params }) {
  ...
  const employee = await auth.models.Employee.findById(id)
    .select('firstName lastName dateOfJoining employmentType lifecycle status dateOfLeaving createdAt')
    .lean()
  ...
  return NextResponse.json({
    ...
    data: {
      ...
      enabled: {
        onboarding: isFeatureEnabled(auth.companyFeatures, 'onboarding'),
...
export async function PATCH(request, { params }) {
  ...
  let result
  try {
    result = applyLifecycleAction(currentLifecycle, action, body, { actorId: actorId(auth.user) })
  } catch (error) {
  ...
  if (workflow) {
    ...
    if (completed) workflow.completedAt = new Date()
  ...
  return NextResponse.json({
    ...
    data: { lifecycle: result.lifecycle, progress: getLifecycleProgress(result.lifecycle) },
  })
}
