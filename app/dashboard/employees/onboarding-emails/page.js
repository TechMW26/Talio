...
export default function OnboardingEmailsPage() {
  ...
  // Retry state (per-email tracking)
  ...
  const swrKey = useMemo(() => {
    ...
    return `/api/employees/onboarding-emails?${params}`
  }, [page, statusFilter, debouncedSearch])

  const { data: swrData, error, isLoading, isValidating, mutate: refreshEmails } = useAuthedSWR(swrKey)

  const emails = swrData?.data || []
  const stats = swrData?.stats || { sent: 0, failed: 0, pending: 0, total: 0 }
  const pagination = swrData?.pagination || { page: 1, limit: 20, total: 0, pages: 0 }
  const autoSendEnabled = swrData?.onboardingEmailsEnabled ?? true

  // Mutations
  const toggleAutoSendMutation = useApiMutation({
    method: 'PATCH',
    invalidateKeys: [/\/api\/employees\/onboarding-emails/],
    onSuccess: (data) => toast.success(data.message),
    onError: (err) => toast.error(err || 'Failed to update setting'),
  })
  ...
  const bulkRetryMutation = useApiMutation({
    method: 'POST',
    invalidateKeys: [/\/api\/employees\/onboarding-emails/],
    onSuccess: (data) => {
      toast.success(data.message)
  ...
  const handleToggleAutoSend = () => {
    ...
    toggleAutoSendMutation.execute('/api/employees/onboarding-emails', { enabled: !autoSendEnabled })
  }
  ...
  const handleSendOnboardingEmail = () => {
    ...
    sendEmailMutation.execute('/api/employees/send-onboarding-email', {
  ...
  const handleBulkRetry = () => {
    ...
    bulkRetryMutation.execute('/api/employees/onboarding-emails/bulk-retry', { emailIds: selectedEmails })
  }

  // Queue all failed emails
  const handleQueueAllFailed = () => {
    if (stats.failed === 0) {
      toast.error('No failed emails to queue')
      return
    }
    queueFailedMutation.execute('/api/employees/onboarding-emails/queue-failed', { delayMinutes: 5 })
  }
          ...
          <div className={`flex items-center gap-3 p-3 rounded-xl border ${autoSendEnabled
            ...
            <button
              ...
              className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2 ${autoSendEnabled ? 'bg-success' : 'bg-default-300'
                } ${toggleAutoSendMutation.isLoading ? 'opacity-50 cursor-wait' : ''} ${user?.role !== 'admin' ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'}`}
      ...
      <div className="flex flex-col sm:flex-row gap-4 mb-6">
        ...
        {selectedEmails.length > 0 && (
          <LoadingButton
            ...
            isLoading={bulkRetryMutation.isLoading}
            loadingText={`Retrying ${selectedEmails.length}...`}
            startContent={<HiOutlineArrowPath className="w-5 h-5" />}
            className="font-medium"
          >
        ...
        {stats.failed > 0 && selectedEmails.length === 0 && (
          <LoadingButton
            ...
            isLoading={queueFailedMutation.isLoading}
            loadingText="Queueing..."
            startContent={<HiOutlineClock className="w-5 h-5" />}
            className="font-medium"
                ...
                {error && !swrData ? (
                  <tr>
                    <td colSpan={8} className="px-4 py-12">
                      <DataErrorState message={error.message} onRetry={() => refreshEmails()} />
                    </td>
                  </tr>
                ) : isLoading ? (
                  // Loading skeleton
        ...
        <ModalContent>
          ...
          <ModalFooter>
            ...
            <LoadingButton
              ...
              loadingText="Sending..."
              isDisabled={!sendEmailAddress.trim()}
              startContent={<HiOutlinePaperAirplane className="w-4 h-4" />}
            >
              Send Email
            </LoadingButton>
          </ModalFooter>
