#!/usr/bin/env node
/**
 * Send onboarding emails to users who haven't received them yet
 * Resets their password and sends new credentials
 * 
 * Usage: node scripts/send-remaining-onboarding.js
 */
...
// Email delay between sends (in ms) to avoid rate limiting
// Hostinger has strict limits - use 15 seconds between emails
const EMAIL_DELAY = 15000;
...
async function sendRemainingOnboardingEmails() {
  console.log('🚀 Starting onboarding email sender...\n');
  ...
  // Get all onboarding email records
  const sentEmails = await onboardingEmails.find({ status: 'sent' }).toArray();
  const sentEmailAddresses = new Set(sentEmails.map(e => e.recipientEmail?.toLowerCase()));
  console.log(`📧 Users with onboarding emails: ${sentEmailAddresses.size}`);
  
  // Find users without onboarding emails
  const usersWithoutEmail = allUsers.filter(user => {
    const email = user.email?.toLowerCase();
    return email && !sentEmailAddresses.has(email);
  });
  
  console.log(`📬 Users needing onboarding email: ${usersWithoutEmail.length}\n`);
  
  if (usersWithoutEmail.length === 0) {
    console.log('✅ All users have received onboarding emails!');
  ...
  for (let i = 0; i < usersWithoutEmail.length; i++) {
    const user = usersWithoutEmail[i];
    const email = user.email;
    
    console.log(`\n[${i + 1}/${usersWithoutEmail.length}] Processing: ${email}`);
    ...
    try {
      ...
      // Log to onboarding emails collection
      await onboardingEmails.insertOne({
      ...
      if (i < usersWithoutEmail.length - 1) {
      ...
      try {
        await onboardingEmails.insertOne({
          user: user._id,
          recipientEmail: email,
          recipientName: user.firstName || 'Unknown',
          status: 'failed',
  ...
  console.log(`📬 Total processed: ${usersWithoutEmail.length}`);
...
// Run the script
sendRemainingOnboardingEmails().catch(e => {
