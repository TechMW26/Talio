...
let lastWhitescreenRecoveryAt = 0;

const LAUNCH_WHITESCREEN_CHECK_DELAYS_MS = [4000, 9000, 15000];
const MIN_WHITESCREEN_RECOVERY_INTERVAL = 60000;
...
function showLoader() {
  ...
  var loaderPath = path.join(__dirname, 'loader.html');
  mainWindow.loadFile(loaderPath).catch(function (error) {
    logger.log('warn', 'Main', 'Failed to load loader page: ' + error.message);
  });
...
function loadApp(options) {
  ...
  try {
    var loadOptions = options.forceFresh ? {
  ...
  mainWindow.webContents.on('did-fail-load', function (event, errorCode, errorDescription, validatedURL, isMainFrame) {
    if (isMainFrame) {
      ...
      if (errorCode === -106) { // ERR_INTERNET_DISCONNECTED
        ...
        logger.log('warn', 'Main', 'File not found during main-frame load, attempting whitescreen recovery...');
        recoverFromWhitescreen('missing-resource').catch(function (recoveryError) {
          logger.log('error', 'Main', 'Whitescreen recovery failed: ' + recoveryError.message);
        });
        return;
      }
  ...
  session.defaultSession.webRequest.onHeadersReceived(
    ...
    function (details, callback) {
      ...
      if (details.resourceType === 'mainFrame') {
        ...
        if (!url.includes('/api/')) {
          // Always revalidate the HTML shell so cached old documents never reference deleted chunks.
...
function clearLaunchWhitescreenChecks() {
  while (launchWhitescreenTimeouts.length > 0) {
    clearTimeout(launchWhitescreenTimeouts.pop());
  }
}

function scheduleLaunchWhitescreenChecks() {
  clearLaunchWhitescreenChecks();

  LAUNCH_WHITESCREEN_CHECK_DELAYS_MS.forEach(function (delayMs) {
    var timer = setTimeout(async function () {
      if (!mainWindow || mainWindow.isDestroyed() || isDownloadingUpdate) return;
      ...
      if (isBlank) {
        logger.log('warn', 'Main', 'Launch whitescreen detected after ' + delayMs + 'ms');
        recoverFromWhitescreen('launch-check-' + delayMs).catch(function (error) {
          logger.log('error', 'Main', 'Launch whitescreen recovery failed: ' + error.message);
        });
      }
    }, delayMs);

    launchWhitescreenTimeouts.push(timer);
  });
}

async function recoverFromWhitescreen(reason) {
  if (!mainWindow || mainWindow.isDestroyed()) return;
  if (isDownloadingUpdate) return;

  var now = Date.now();
  if (now - lastWhitescreenRecoveryAt < MIN_WHITESCREEN_RECOVERY_INTERVAL) {
    logger.log('info', 'Main', 'Skipping whitescreen recovery — last attempt was too recent');
    return;
  }

  lastWhitescreenRecoveryAt = now;
  clearLaunchWhitescreenChecks();
  clearTimeout(loadTimeout);
  clearTimeout(navigationSafetyTimer);
  clearTimeout(pendingReloadTimeout);
  ...
  try {
    ...
    logger.log('warn', 'Main', 'clearCache during whitescreen recovery failed: ' + error.message);
  }
  ...
  try {
    ...
    logger.log('warn', 'Main', 'clearStorageData during whitescreen recovery failed: ' + error.message);
  }
...
function setupNetworkMonitoring() {
  ...
  const checkAndRecoverWhitescreen = async function () {
    ...
    if (isBlank) {
      ...
      recoverFromWhitescreen('periodic-check').catch(function (error) {
        logger.log('error', 'Main', 'Periodic whitescreen recovery failed: ' + error.message);
      });
    }
  };
