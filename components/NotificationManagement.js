          ...
          <div>
            ...
            <div className="border border-gray-300 rounded-lg p-4 space-y-2 bg-gray-50">
              {availableRoles.map(role => (
                <label key={role.name} className="flex items-center space-x-3 p-2 hover:bg-white rounded-lg cursor-pointer transition-colors">
                  <input
                    type="checkbox"
                  ...
                  />
                  <span className="text-sm font-medium text-gray-900 capitalize flex-1">{role.displayLabel}</span>
                </label>
              ))}
            </div>
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex items-center flex-wrap gap-1 text-xs text-gray-500 mt-0.5">
                              ...
                              {emp.userId?.role && (
                                <>
                                  <span>•</span>
                                  <span className="capitalize">{getRoleDisplayLabel(emp.userId.role, availableRoles)}</span>
                                </>
                              )}
                            </div>
                          </div>
...
function ScheduledNotificationsTab({ userRole, userDepartment, isDepartmentHead }) {
  ...
  const getTargetLabel = (notification) => {
    ...
    if (notification.targetType === 'role') return `Roles: ${notification.targetRoles?.map(r => getRoleDisplayLabel(r, availableRoles)).join(', ') || 'N/A'}`
            ...
            {availableRoles.map(role => (
              <label key={role.name} className="flex items-center space-x-3 p-2 hover:bg-white rounded-lg cursor-pointer transition-colors">
                <input
                  ...
                  className="rounded text-primary-500 w-4 h-4 focus:ring-primary-500"
                />
                <span className="text-sm font-medium text-gray-900 capitalize">{role.displayLabel}</span>
              </label>
            ))}
          </div>
        </div>
              ...
              <div className="flex justify-between items-start">
                <div className="flex-1">
                  <div className="flex items-center space-x-2">
                    ...
                    <span className={`px-2 py-1 text-xs rounded-full ${notification.isActive ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'
                      }`}>
                ...
                <div className="ml-4 flex space-x-2">
                  <button
                    ...
                    className={`px-3 py-1 text-sm rounded-lg transition-colors flex items-center space-x-1 ${notification.isActive
...
function ConfigurationTab({ apiKeyConfigured, onConfigUpdate }) {
  ...
  return (
    <div className="max-w-3xl">
      {/* Status Banner */}
      <div className={`mb-6 p-4 rounded-lg border-l-4 ${apiKeyConfigured
        ...
        <div className="flex items-center">
          ...
          <div className="ml-3">
            <h3 className={`text-sm font-medium ${apiKeyConfigured ? 'text-green-800' : 'text-yellow-800'
              }`}>
              {apiKeyConfigured ? 'Firebase Cloud Messaging is Configured' : 'Firebase Not Configured'}
            </h3>
            <p className={`mt-1 text-sm ${apiKeyConfigured ? 'text-green-700' : 'text-yellow-700'
              }`}>
              {apiKeyConfigured
