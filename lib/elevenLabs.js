...
function getElevenLabsConfig() {
  // Try both NEXT_PUBLIC_ and server-side env vars for flexibility
  const apiKey = process.env.NEXT_PUBLIC_ELEVENLABS_API_KEY || process.env.ELEVENLABS_API_KEY;
  const voiceId = process.env.NEXT_PUBLIC_ELEVENLABS_VOICE_ID || process.env.ELEVENLABS_VOICE_ID || 'm7GHBtY0UEqljrKQw2JH';
  const apiUrl = process.env.NEXT_PUBLIC_ELEVENLABS_API_URL || process.env.ELEVENLABS_API_URL || 'https://api.elevenlabs.io/v1/text-to-speech/';
