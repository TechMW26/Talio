...
function getElevenLabsConfig() {
  ...
  const speechToTextApiUrl = process.env.ELEVENLABS_SPEECH_TO_TEXT_API_URL || 'https://api.elevenlabs.io/v1/speech-to-text';
  const transcriptionModelId = process.env.ELEVENLABS_TRANSCRIPTION_MODEL_ID || 'scribe_v2';
  ...
  console.log('[ElevenLabs Config] Speech-to-Text URL:', speechToTextApiUrl);
  
  return { apiKey, voiceId, apiUrl, speechToTextApiUrl, transcriptionModelId };
}
 ...
 * @returns {Promise<{success: boolean, text?: string, languageCode?: string, words?: Array, raw?: object, error?: string}>}
 */
export async function transcribeAudio(file, options = {}) {
  ...
  try {
    ...
    const response = await fetch(config.speechToTextApiUrl, {
      method: 'POST',
      headers: {
        'xi-api-key': config.apiKey,
      },
    ...
    if (!response.ok) {
      ...
      return {
        success: false,
        error: `ElevenLabs transcription error: ${response.status} - ${errorText}`,
      };
    }
