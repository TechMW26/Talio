...
export async function ensureEmployeeLeaveBalances({ models, employeeId, year = new Date().getFullYear() }) {
  ...
  const employee = await Employee.findById(employeeId).select('_id status dateOfJoining').lean()
      ...
      updateOne: {
        ...
        update: {
          $setOnInsert: {
            ...
            year,
            ...buildLeaveBalanceFields({ totalDays: prorateAnnualLeave(normalized.maxDaysPerYear, employee.dateOfJoining, year) }),
          },
        },
        upsert: true,
      },
