  ...
  async emit(event, data = {}, acknowledge) {
    try {
      ...
      if (event === 'send-message') {
        // The message API already publishes the persisted record.
        acknowledge?.(null, { success: true })
        return this
      }
      if (!['typing', 'stop-typing', 'mark-read'].includes(event)) return this
      const response = await fetch('/api/realtime/events', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', ...(this.token ? { Authorization: `Bearer ${this.token}` } : {}) },
        body: JSON.stringify({ event, chatId: data.chatId, messageId: data.messageId }),
      })
      if (!response.ok) throw new Error('Realtime event could not be delivered')
      acknowledge?.(null, { success: true })
    } catch (error) {
      acknowledge?.(error)
...
export function createRealtimeClient({ token, userId, employeeId, tenantId }) {
