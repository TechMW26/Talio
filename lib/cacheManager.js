...
export async function checkAndClearCaches() {
  ...
  // CRITICAL: Never clear caches or reload in Electron - causes white screen
  if (isElectronApp()) {
    console.log('[CacheManager] Desktop app detected, skipping cache operations');
    return false;
  }
...
export async function forceClearAndReload() {
  ...
  // CRITICAL: Never reload in Electron - causes white screen
  if (isElectronApp()) {
    console.log('[CacheManager] Desktop app detected, skipping force reload');
    return;
  }
