...
export async function startMiraConversationRecognition({ signal, onResult, onPartial, onError }) {
  ...
  try {
    check()
    // Download/compile the capture worklet while permission and token acquisition
    // are pending, instead of adding another round trip after socket startup.
    const workletReady = prepareMiraAudioCapture(context)
    // Attach rejection handlers immediately while microphone permission is pending.
    const audioReady = Promise.all([ready, workletReady]).then(() => null, error => error)
    ...
    check()
    const useWorklet = await workletReady
    check()
    processor = createMiraAudioCapture(context, useWorklet, samples => {
    ...
    source.connect(processor.node); processor.node.connect(context.destination)
