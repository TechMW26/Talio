...
import { SCREENSHOTS_PER_SESSION, SESSION_DURATION_MINUTES as SESSION_DURATION_MINS } from '@/lib/productivitySessionRules';

// SESSION CONFIGURATION
// Each full batch contains 20 screenshots; capture cadence is configured separately.
 ...
 * Create sessions of exactly 20 screenshots each
 * - Start from first screenshot of the day
 * - Collect 20 consecutive screenshots into one session
 * - Last session of day ends at checkout (may have < 20 screenshots)
 * 
...
export async function aggregateSessionsForUser(userId, targetDate = new Date(), forceRecompile = false) {
  ...
  // Group data into sessions of 20 screenshots each
  const sessions = [];
  let currentBatch = [];
  let sessionIndex = 0;
  ...
  for (let i = 0; i < rawData.length; i++) {
    ...
    // Check if we have 20 screenshots OR this is the last data point before checkout
