...
async function invalidateActionableNotificationCache(ActionableNotification, userIds) {
  const tenantId = ActionableNotification?.db?.name
...
export async function dismissNotificationsForReference(models, referenceModel, referenceId) {
  ...
  if (result.modifiedCount > 0) {
    ...
    await invalidateActionableNotificationCache(ActionableNotification, notifications.map(n => n.user))
 ...
 */
export async function createProbationApprovalNotification(models, params) {
  const { ActionableNotification } = models
  const {
    targetUserId,
    approvalId,
  ...
  await invalidateActionableNotificationCache(ActionableNotification, [targetUserId])
  if (global.io) {
    global.io.to(`user:${targetUserId}`).emit('actionable-notification', {
      notification: notification.toObject(),
    })
