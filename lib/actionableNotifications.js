...
export async function createTaskAssignmentNotification(models, params) {
  ...
  const notification = await ActionableNotification.create({
    ...
    actions: [
      {
        ...
        label: 'Accept',
        variant: 'success',
        endpoint: projectId ? `/api/projects/${projectId}/tasks/${taskId}/respond` : `/api/tasks/${taskId}/respond`,
        method: 'POST',
        payload: { action: 'accept' }
      },
      {
    ...
    url: projectId ? `/dashboard/projects/${projectId}?task=${taskId}` : `/dashboard/todo`,
