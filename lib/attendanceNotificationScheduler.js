    ...
    for (const attendance of stillCheckedIn) {
      try {
        ...
        if (attendance.checkIn && settings?.breakTimings) {
          ...
          const hoursWorked = (scheduledCheckoutTime - new Date(attendance.checkIn)) / (1000 * 60 * 60)
...
export async function processPastDayIncompleteAttendance(settings, filter = {}) {
  try {
    ...
    let incompleteAttendance = await Attendance.find(buildOpenAttendanceQuery({
      targetDateStart: new Date(0),
      targetDateEnd: new Date(todayStart.getTime() - 1),
      includeBacklog: true,
    })).populate({
      path: 'employee',
      match: filter,
    ...
    for (const record of incompleteAttendance) {
      try {
        ...
        const autoCheckoutTime = resolveScheduledCheckout({
          attendanceDate,
          checkIn: attendance.checkIn,
          checkOutTime: `${String(endHour).padStart(2, '0')}:${String(endMin).padStart(2, '0')}`,
          timezone: settings?.timezone || 'Asia/Kolkata',
        })
        if (!autoCheckoutTime) continue
