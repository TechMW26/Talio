...
import { miraDesktopIntent } from './miraDesktopIntent'
  ...
  try {
    for (let step = 0; step < 24; step++) {
      ...
      if (!observation.success) {
        if (observation.retryable && recoveries++ < 2) continue
        return observation
      }
      let result
      ...
      if (!outcome.success) {
        if (outcome.retryable && recoveries++ < 2) continue
        return outcome
      }
      recoveries = 0
    ...
    await api({ operation: 'cancel', sessionId }).catch(() => {})
