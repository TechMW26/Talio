...
export async function sendAndLogOnboardingEmail({
  ...
  // Use tenant models if provided, else fallback to default imports
  let OnboardingEmail, CompanySettings

  if (models.OnboardingEmail && models.CompanySettings) {
    OnboardingEmail = models.OnboardingEmail
    CompanySettings = models.CompanySettings
  } else {
    // Dynamic import for backward compatibility (cron jobs, scripts, etc.)
    const { default: DefaultOnboardingEmail } = await import('@/models/OnboardingEmail')
    const { default: DefaultCompanySettings } = await import('@/models/CompanySettings')
    const { default: connectDB } = await import('@/lib/mongodb')
    await connectDB()
    OnboardingEmail = DefaultOnboardingEmail
  ...
  const ALLOWED_TRIGGERED_BY = new Set(['manual_creation', 'bulk_import', 'manual_retry', 'manual_script'])
  const normalizedTriggeredBy = ALLOWED_TRIGGERED_BY.has(triggeredBy) ? triggeredBy : 'manual_creation'
  ...
  try {
    ...
    if (result.success) {
      ...
      // Check if rate limited - queue for automatic retry
      if (isRateLimitError(result.error)) {
        emailLog.status = 'pending'
        emailLog.queued = true
        emailLog.autoRetryCount = 1
        emailLog.scheduledFor = calculateNextRetryTime(1)
        emailLog.rateLimitedUntil = emailLog.scheduledFor
        emailLog.errorMessage = `Rate limited. Auto-retry scheduled for ${emailLog.scheduledFor.toISOString()}`
 ...
 * MULTI-TENANT: Pass models parameter with { OnboardingEmail } for tenant-aware operations
 */
export async function retryOnboardingEmail(emailLogId, retriedByUserId = null, models = {}) {
  let OnboardingEmail

  if (models.OnboardingEmail) {
    OnboardingEmail = models.OnboardingEmail
  } else {
  ...
  // Update retry info — clear queued state so manual retry takes over
