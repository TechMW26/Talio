/**
 * Department Head Sync - Syncs department head status to User meta
 * 
 * MULTI-TENANT: This module now supports tenant-aware models.
 * Pass tenant models via options parameter when called from authenticated routes.
 * Falls back to default models for backward compatibility (cron jobs, etc.)
 */

import DefaultUser from '@/models/User';
import DefaultEmployee from '@/models/Employee';
import DefaultDepartment from '@/models/Department';
 ...
 * @param {Object} models - Optional tenant-aware models { User, Employee, Department }
 * @returns {Promise<{success: boolean, updated: number, message: string}>}
 */
export async function syncDepartmentHeadStatus(employeeId = null, models = {}) {
  try {
    const User = models.User || DefaultUser;
    const Employee = models.Employee || DefaultEmployee;
    const Department = models.Department || DefaultDepartment;

    // Only connect to default DB if no models provided
    if (!models.User) {
      await connectDB();
    }
 ...
 * @param {Object} models - Optional tenant-aware models { User, Employee, Department }
 */
export async function updateDepartmentHeadsForDepartment(departmentId, previousHeads = [], newHeads = [], models = {}) {
  try {
    const User = models.User || DefaultUser;
    const Department = models.Department || DefaultDepartment;
    ...
    for (const empId of removedHeads) {
      await syncDepartmentHeadStatus(empId, models);
    }
 ...
 */
export async function getAllDepartmentHeads(models = {}) {
  try {
    const User = models.User || DefaultUser;
