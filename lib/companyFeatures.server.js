...
export async function getTenantCompanyFeaturePayload({ companySlug, databaseName }) {
  ...
  // The database result is authoritative; a slow remote cache must not extend
  // the critical path of every authenticated API request.
  void setCache(cacheKey, response, 300).catch(() => {})
  return response
}
