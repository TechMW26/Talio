...
async function syncHolidaysForTenant(Holiday, events, year) {
    ...
    for (const event of events) {
        ...
        try {
            // Find existing by name (case-insensitive) + same date
            ...
            if (existing) {
                // Update existing holiday with latest Google data
    ...
    // Clean up any non-public holidays previously synced from Google Calendar
    try {
        ...
        if (cleanupResult.deletedCount > 0) {
            console.log(`  🧹 [HolidaySync] Removed ${cleanupResult.deletedCount} non-public google-calendar holidays`);
        }
    } catch (err) {
        console.warn(`  ⚠️ [HolidaySync] Cleanup error:`, err.message);
    }
