...
function lockTopLevelClaimsToEvidence(analysis) {
    ...
    // Only derive from screenshots when the AI did not already provide valid data.
    // When the AI returns applications/websites with real estimatedMinutes values,
    // those must be preserved instead of being replaced by occurrence counts.
...
function ensureStrictPointers(analysis) {
    ...
    // Only overwrite suggestions when the AI did not already provide context-aware ones.
    // Preserving AI suggestions is critical because they feed into areasForImprovement
    // in the overallAssessment via buildOverallAssessment().
