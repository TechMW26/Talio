...
export async function listDirectory({
  ...
  const cacheKey = buildCacheKey({
    ...
    params: { normalizedQuery, safeLimit, safePage, includeAdmins, includeSelf, version: 2 },
  })
  const cached = await getCache(cacheKey)
  if (cached) return cached

  const currentUser = await User.findById(currentUserId).select('employeeId').lean()
  const employeeFilter = { status: { $in: ['active', 'probation', 'on_leave'] } }
  if (!includeSelf && currentUser?.employeeId) employeeFilter._id = { $ne: currentUser.employeeId }
  // Exclude before pagination, otherwise filtered pages can look prematurely empty.
  if (!includeAdmins) {
    const admins = await User.find({ role: 'admin', isActive: true }).select('employeeId').lean()
    employeeFilter._id = { ...employeeFilter._id, $nin: admins.map(user => user.employeeId).filter(Boolean) }
  }
  if (normalizedQuery) {
