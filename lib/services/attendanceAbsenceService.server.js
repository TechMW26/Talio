...
export async function processAbsenceDate({ models, date, dryRun = false, sendNotifications = false }) {
  ...
  }).select('name type dayPortion').lean()
  if (holiday && (!holiday.dayPortion || holiday.dayPortion === 'full_day')) {
    return { ...result, skipped: true, skipReason: 'holiday', holiday }
  }

  const [employees, leaves, attendance] = await Promise.all([
    models.Employee.find({ status: { $in: ['active', 'probation'] } }).select('_id firstName lastName dateOfJoining userId').populate('userId', '_id').lean(),
    models.Leave.find({
      status: 'approved',
...
export async function getAbsenceStatus({ models, date }) {
  ...
  const [holiday, attendanceStats, systemGenerated, totalEmployees, onLeave] = await Promise.all([
    models.Holiday.findOne({ isActive: true, $or: [{ date: { $gte: dayStart, $lte: dayEnd } }, { date: { $lte: dayStart }, endDate: { $gte: dayStart } }] }).select('name type dayPortion').lean(),
    ...
    models.Employee.countDocuments({ status: { $in: ['active', 'probation'] } }),
  ...
  return {
    ...
    isWorkingDay: workDay, configuredWorkingDays: calendar.workingDays, isHoliday: Boolean(holiday), isFullDayHoliday: Boolean(holiday && (!holiday.dayPortion || holiday.dayPortion === 'full_day')), holiday,
    totalEmployees, onLeave, attendance, systemGenerated, userGenerated: accounted - systemGenerated,
    unaccounted: workDay && !(holiday && (!holiday.dayPortion || holiday.dayPortion === 'full_day')) ? Math.max(0, totalEmployees - accounted - onLeave) : 0,
  }
}
