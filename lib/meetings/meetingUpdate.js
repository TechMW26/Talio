...
export function buildMeetingReminders(reminders, scheduledStart) {
  const configured = Array.isArray(reminders) && reminders.length
    ? reminders
    : [{ type: '15min' }]
  return configured.map((reminder) => {
    const type = REMINDER_OFFSETS[reminder?.type] ? reminder.type : '15min'
    return {
      type,
      time: new Date(scheduledStart.getTime() - REMINDER_OFFSETS[type]),
      sent: false,
      sentAt: null,
    }
  })
...
export function buildMeetingDetailsUpdate(data, meeting, {
  ...
  return {
    ...
    duration,
    reminders: buildMeetingReminders(meeting.reminders, scheduledStart),
  }
}
