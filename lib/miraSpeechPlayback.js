...
export function createMiraSpeechPlayback() {
  ...
  return {
    ...
    async speak(text, onStart) {
      ...
      const response = await fetchMiraAudio({ method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` }, body: JSON.stringify({ text: text.slice(0, 5000) }), signal: own.signal })
