...
export async function resolveMiraPerson(name, user, models, { field, type } = {}) {
  ...
  const broad = ['admin', 'hr'].includes(user.role) || ['send_message', 'create_meeting', 'lookup_people'].includes(type)
  const scope = broad ? {} : { $or: [{ _id: own }, buildDirectReportsFilter(own)] }
  ...
  const matches = await models.Employee.find({ $and: [scope, { status: 'active' }, selected ? { _id: selected[1] } : { $or: clauses }] })
  ...
  const error = new Error(matches.length ? `Choose the person you mean by “${name}”.` : `No matching person found for “${name}” within your available contacts. Try another spelling or employee code.`)
