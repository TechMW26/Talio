...
// Strip markdown emphasis artifacts (e.g. ``**`` bold/italic markers) that the
// model sometimes leaks into JSON value positions. Operates only outside
// string literals so legitimate ``**`` characters inside string content are
// preserved untouched.
function stripMarkdownArtifacts(text) {
    ...
    while (i < text.length) {
        ...
        // Bare ``**`` placeholder in a value position (next non-whitespace is
        // a structural delimiter) becomes a JSON ``null`` so parsing succeeds.
        ...
        // ``**Some Title**`` style: capture the inner phrase up to the next
        // run of asterisks and emit it as a quoted JSON string.
        const wrapped = text.slice(j).match(/^([^*"\n\r,}\]]+?)\*+/);
        if (wrapped) {
            const inner = wrapped[1].trim();
            result += JSON.stringify(inner);
            i = j + wrapped[0].length;
            continue;
        }

        // ``**"already quoted"**`` — emit the inner quoted string as-is.
        const wrappedQuoted = text.slice(j).match(/^\s*("(?:[^"\\]|\\.)*")\s*\*+/);
        if (wrappedQuoted) {
            result += wrappedQuoted[1];
            i = j + wrappedQuoted[0].length;
            continue;
        }

        // Stray asterisks with no recognizable pattern — drop them silently.
        i = j;
    }
...
function tryParseCandidate(candidate) {
    ...
    const markdownStripped = stripMarkdownArtifacts(sanitized);
    if (markdownStripped !== sanitized) {
        const markdownParsed = tryParseJson(markdownStripped);
        if (markdownParsed) {
            return { parsed: markdownParsed, parseError: null };
        }
    }

    // Prefer the purpose-built repair library before applying the legacy
    // token-based recovery below. AI responses commonly contain unescaped
    // quotes inside strings, missing commas, or truncated closing brackets;
    // jsonrepair handles those combinations without discarding valid sections.
    try {
        const repaired = jsonrepair(markdownStripped);
        const repairedParsed = tryParseJson(repaired);
        if (repairedParsed) {
            return { parsed: repairedParsed, parseError: null };
        }
    } catch {
        // Continue through the existing best-effort recovery strategies.
    }

    const structurallyRepaired = repairStructuralDelimiters(markdownStripped);
