...
export async function analyzeProductivityData(data) {
  ...
  // Try Gemini first via promptEngine, then fallback to centralized AI
  try {
    return await analyzeWithGemini(context, data, userId);
  } catch (error) {
    console.error('[ProductivityAnalyzer] Gemini via promptEngine failed, trying fallback AI:', error.message);
  }

  try {
    return await analyzeWithFallbackAI(context, data);
  } catch (error) {
    console.error('[ProductivityAnalyzer] Fallback AI failed:', error.message);
  }
...
ROLE CONTEXT (IMPORTANT FOR FAIR SCORING):
${roleContext}
`.trim();
}

function buildRoleContext(data) {
  const ctx = data?.employeeContext || {};
  const designation = ctx.designation || data?.designation || 'Not provided';
  const department = ctx.department || data?.department || 'Not provided';

  const kriItems = [];
  if (Array.isArray(ctx.manualKRIs)) kriItems.push(...ctx.manualKRIs);
  if (Array.isArray(ctx.aiGeneratedKRIs)) {
    kriItems.push(...ctx.aiGeneratedKRIs.map((k) => (typeof k === 'string' ? k : k?.title)).filter(Boolean));
  }

  const kriList = kriItems.filter(Boolean).slice(0, 10);

  return `Designation: ${designation}
Department: ${department}
KRIs: ${kriList.length > 0 ? kriList.join('; ') : 'No explicit KRIs provided. Infer expected work from designation.'}`;
}
...
async function analyzeWithGemini(context, data, userId) {
  const prompt = `You are a productivity analyst AI. Analyze this employee's computer usage data and provide evidence-based insights.

${context}

SCORING RULES:
- Base productivityScore primarily on productive vs unproductive time.
- Keep scores calibrated: do not over-penalize for missing data.
- If productive time is high and unproductive time is low, avoid low scores.
- If data quality is low (very short session or sparse inputs), reflect uncertainty in insights instead of extreme scores.
- Keep productivityScore, focusScore, and efficiencyScore mutually consistent.
- Evaluate work relevance by employee designation/KRIs (example: a video editor researching YouTube references can be productive work).
...
/**
 * Analyze with the centralized custom AI provider.
 */
async function analyzeWithFallbackAI(context, data) {
  const systemInstruction = 'You are a productivity analyst AI. Analyze employee computer usage data with evidence-based, calibrated scoring. Always respond with valid JSON only.';

  const prompt = `${context}

SCORING RULES:
- Base productivityScore mostly on productive vs unproductive time ratio.
- Keep focusScore tied to context switching/distraction evidence.
- Keep efficiencyScore tied to engagement signals (keystrokes/clicks) without punishing missing data too harshly.
- Avoid contradictory outputs (for example high productive ratio with very low productivityScore).
- Evaluate activity against role context and KRIs before labeling it unproductive.
...
Respond with a JSON object:
{
  ...
  const text = await generateContent(prompt, systemInstruction, { useCase: 'analysis' });
 ...
 * Uses centralized gemini.js with the custom AI vision provider.
 */
export async function analyzeScreenshot(screenshotBase64) {
  try {
    ...
    const prompt = 'Briefly describe what work this person is doing based on their screen. Keep it professional and factual. Also estimate a productivity score 0-100. Respond in JSON: {"summary": "...", "productivityScore": number, "applications": ["app1", "app2"]}';

    const result = await generateVisionContent(prompt, [{ data: imageData, mimeType }]);
