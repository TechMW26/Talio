...
const REMOTE_CACHE_BUDGET_MS = Math.max(
  100,
  Number.parseInt(process.env.REDIS_OPERATION_TIMEOUT_MS || '750', 10) || 750
)
...
export async function getCache(key) {
  if (await shouldBypassRequestCache()) {
    return null
  }
  ...
  // L2: Redis
  try {
    const client = await withinRemoteCacheBudget(getRedisClient())
    if (client === CACHE_TIMEOUT) markRemoteCacheTimedOut()
    if (client && client !== CACHE_TIMEOUT) {
      const value = await withinRemoteCacheBudget(client.get(key))
      if (value === CACHE_TIMEOUT) {
...
export async function setCache(key, value, ttlSeconds = 60) {
  ...
  try {
    ...
    if (client === CACHE_TIMEOUT) markRemoteCacheTimedOut()
    if (client && client !== CACHE_TIMEOUT) {
      const result = await withinRemoteCacheBudget(
        client.set(key, JSON.stringify(value), { EX: ttlSeconds })
      )
      if (result !== CACHE_TIMEOUT) return
      markRemoteCacheTimedOut()
    }
  } catch {
    // Silently fall back to memory cache
