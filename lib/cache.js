    ...
    try {
      ...
      const client = createClient({
        ...
        socket: {
          ...
          keepAlive: isProduction ? 30000 : false, // Keep connections alive in production
          reconnectStrategy: (retries) => {
            // More aggressive reconnection in production
            if (retries > (isProduction ? 10 : 3)) {
              global.__redisConnectionFailed = true
              return false
            }
            return Math.min(retries * 100, isProduction ? 3000 : 1000)
          }
        },
