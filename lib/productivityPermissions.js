...
export async function canViewUserScreenshots(viewerId, targetUserId, viewerRole, models) {
  ...
  const targetEmployee = await Employee.findById(target.employeeId).select('department departments assignedManager assignedTeamLead reportsTo reportingManager');
