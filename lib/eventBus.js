...
import { clearCachePattern, deleteCache, buildCacheKey } from './cache.js'
...
async function invalidateCachesForEvent(event, payload, { userIds, databaseName }) {
  ...
  switch (event) {
    ...
    case EVENTS.CHAT_MESSAGE_SENT: {
      // Invalidate chat:unread cache for affected users
    ...
    case EVENTS.PROJECT_INVITATION_CHANGED: {
      // Invalidate sidebar:counts cache for affected users
      for (const userId of userIds) {
        // Sidebar counts are keyed by role, but we don't know the role here.
        // Use pattern match to clear all role variants for this user.
        const pattern = `${databaseName}:*:${userId}:sidebar:counts`
        promises.push(clearCachePattern(pattern))
      }
      break
    }
