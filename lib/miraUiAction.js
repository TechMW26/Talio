...
export async function executeMiraUiAction(action) {
  ...
  window.dispatchEvent(new CustomEvent('mira:activity', { detail: { label: `Opening ${target}`, phase: 'done', target: element } }))
  await new Promise(resolve => setTimeout(resolve, 320))
  if (!element.isConnected || element.disabled) return { success: false, message: 'The page changed. Please try again.' }
  element.click()
