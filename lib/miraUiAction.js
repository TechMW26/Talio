...
export function getMiraUiContext() {
  ...
  const controls = [...root.querySelectorAll('a[href],button,[role="tab"],input:not([type="password"]),select,textarea')].filter(visibleControl).map(el => {
...
export async function executeMiraUiAction(action, { resolveSnapshot, signal } = {}) {
  ...
  const dialog = [...document.querySelectorAll('[role="dialog"],dialog[open]')].find(visibleControl)
  const candidates = [...(dialog || document).querySelectorAll('a[href],button,[role="tab"]')].filter(isMiraNavigationControl)
