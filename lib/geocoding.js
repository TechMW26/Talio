 ...
 * Geocoding Utility
 * 
 * Uses OpenStreetMap Nominatim for reverse geocoding (coordinates to address)
...
export async function reverseGeocode(latitude, longitude) {
    try {
        ...
        // Build complete pinpoint address with all available details
        ...
        // Construct final address with all details
        let formattedAddress = addressParts.filter(Boolean).join(', ');
        if (pincode) {
            formattedAddress += ` - ${pincode}`;
        }

        // If we couldn't build a detailed address, use Nominatim's display_name which is very detailed
