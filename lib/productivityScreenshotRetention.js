...
export async function cleanupExpiredScreenshotsForTenant({ databaseName, models, cutoff = getScreenshotRetentionCutoff() }) {
    ...
    try {
        const orphanResult = await deleteOldScreenshots(cutoff, { databaseName });
        gridfsDeleted += orphanResult?.deletedCount || 0;
    } catch (error) {
        console.error('[ScreenshotRetention] Orphan GridFS cleanup failed:', error.message);
    }
