...
export function getVercelReadiness(env = process.env) {
  ...
  // GridFS is the existing tenant-scoped durable upload store. Blob is optional,
  // not a reason to report an otherwise healthy Mongo-backed deployment down.
  if (!env.BLOB_READ_WRITE_TOKEN && !env.MONGODB_URI) {
    missing.push({ capability: 'private object storage', missingKeys: ['BLOB_READ_WRITE_TOKEN or MONGODB_URI'] })
  }
  const invalid = []
