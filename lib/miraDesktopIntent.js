...
export function miraOpenAppIntent(goal) {
  const match = String(goal || '').trim().match(/^(?:please\s+)?(?:open|launch|focus|bring up|switch to)\s+(?:the\s+)?(whats\s*app|telegram|signal|slack|outlook|gmail|discord|spotify|notes|calculator|finder|textedit|chrome|safari|notepad)(?:\s+(?:app|application))?[.!]?$/iu)
