...
export async function verifyTokenFromRequest(request) {
  try {
    ...
    // Use tenant-specific User and Employee models - NEVER fall back to default DB
    // We need Employee model for populate to work
    const models = await getTenantModels(tenantInfo.databaseName, ['User', 'Employee']);
    const { User: UserModel } = models;

    // Fetch user from the tenant's database (use lean for faster reads)
