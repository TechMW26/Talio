...
export async function checkAndTriggerSessionAnalysis({ userId, employeeId, databaseName, capturedAt, models }) {
    try {
        ...
        // Calculate the 3-hour block for this screenshot (non-overlapping)
        // Blocks: 00:00-03:00, 03:00-06:00, 06:00-09:00, 09:00-12:00, 12:00-15:00, 15:00-18:00, 18:00-21:00, 21:00-24:00
        const hourOfDay = capturedAt.getHours();
        const blockStartHour = Math.floor(hourOfDay / 3) * 3;
        const windowStart = new Date(capturedAt);
        windowStart.setHours(blockStartHour, 0, 0, 0);
        ...
        const screenshots = await Screenshot.find({
            ...
            .sort({ capturedAt: 1 })
            .select('gridfsFileId path capturedAt filename captureType')
            .lean();
        ...
        if (session) {
            ...
            if (session.screenshotCount !== screenshots.length) {
                session.screenshots = screenshots.map(ss => ({
                    path: ss.gridfsFileId ? `/api/images/${ss.gridfsFileId}` : ss.path,
                    url: ss.gridfsFileId ? `/api/images/${ss.gridfsFileId}` : ss.path,
                    fileId: ss.gridfsFileId?.toString() || null,
