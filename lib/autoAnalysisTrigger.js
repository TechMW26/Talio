 ...
 * Two triggers:
 * 1. Session reaches 20 screenshots (full hourly session) — called after each screenshot upload
...
const SCREENSHOTS_PER_SESSION = 20;

/**
 * Check if a session is ready for auto-analysis after a screenshot upload.
 * Groups screenshots into 60-minute hourly windows and creates/updates
 * ProductivitySession documents. When a session hits 20 screenshots, enqueues it.
 * 
 * @param {Object} params
...
export async function triggerAnalysisOnCheckout({ userId, employeeId, databaseName, models }) {
    try {
        ...
        // (the partial last session that didn't reach 20 screenshots)
