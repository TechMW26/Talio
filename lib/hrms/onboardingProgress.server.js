...
export async function getOnboardingCompletionSignals({ models, employee }) {
  ...
  // Aadhaar is the only mandatory employee identity document. PAN, offer,
  // experience and other records remain available, but do not block onboarding.
  const hasIdentityProof = hasMatchingDocument(allDocuments, /aadhaar|aadhar/)
