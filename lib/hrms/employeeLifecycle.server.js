...
import { createLinkedOnboardingVerification, normalizeOnboardingVerification } from '@/lib/hrms/onboardingVerification'
...
export function reconcileOnboardingChecklist(lifecycle, signals = {}, context = {}) {
  ...
  for (const item of next.onboarding.checklist) {
    ...
    item.verification = createLinkedOnboardingVerification(item.key, { now })
...
export function applyLifecycleAction(lifecycle, action, payload = {}, context = {}) {
  ...
  if (action === 'complete_onboarding_item') {
    ...
    if (item.completed) {
      const normalized = normalizeOnboardingVerification(item.key, payload.verification, {
        actorId,
      ...
      item.verification = normalized.verification
      Object.assign(employeeUpdates, normalized.employeeUpdates)
    } else {
      const reason = String(payload.reason || '').trim()
      if (!reason) throw new Error('A reason is required to reopen a verified step')
      item.completedAt = null
      item.completedBy = null
      item.completionSource = null
      item.verification = {
        ...(item.verification || {}),
        status: 'reopened',
        reopenedAt: now,
        reopenedBy: actorId,
        reopenReason: reason.slice(0, 1000),
      }
    }
    ...
    next.probation.pip = payload.pip || null
  ...
  } else if (action === 'save_onboarding_letter') {
    if (!['offer', 'appointment'].includes(payload.kind)) throw new Error('Choose an offer or appointment letter')
    const content = String(payload.content || '').trim()
    if (!content || content.length > 20000) throw new Error('Letter must contain between 1 and 20000 characters')
    next.letterDraft = { kind: payload.kind, content, savedAt: now }
  } else if (action === 'save_settlement') {
    if (!next.offboarding || next.offboarding.status === 'not_started' || next.offboarding.status === 'completed') throw new Error('An active offboarding is required')
    next.offboarding.settlement = { ...validateSettlement(payload.settlement), savedAt: now }
    next.offboarding.fullAndFinalStatus = 'pending'
  } else if (action === 'update_offboarding') {
    if (payload.field === 'assetsReturned') throw new Error('Use the connected asset clearance checklist to return assets')
    const allowed = new Set(['exitInterviewCompleted', 'accessRevoked', 'fullAndFinalStatus', 'experienceLetterStatus'])
