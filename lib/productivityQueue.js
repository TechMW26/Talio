...
import { deleteScreenshots as deleteGridFSScreenshots, getScreenshot as getGridFSScreenshot, getScreenshotInfo } from '@/lib/gridfs';
...
async function analyzeSession(entry) {
    ...
    for (const screenshot of selectedScreenshots) {
        try {
            ...
            // Priority 1: Load from GridFS via fileId
            const gridfsId = screenshot.fileId || screenshot.gridfsFileId;
            if (gridfsId) {
                try {
                    const info = await getScreenshotInfo(gridfsId);
                    if (info?.contentType) mimeType = info.contentType;
                    const buffer = await getGridFSScreenshot(gridfsId);
                    base64 = buffer.toString('base64');
                    images.push({ mimeType, data: base64 });
                    continue;
                } catch (gridfsErr) {
                    console.warn(`[ProductivityQueue] GridFS load failed for ${gridfsId}, trying URL fallback:`, gridfsErr.message);
                }
            }

            // Priority 2: Load from URL or filesystem path
            ...
            } else if (screenshotUrl.startsWith('/api/images/')) {
                // Internal GridFS URL — extract ID and load directly
                const extractedId = screenshotUrl.replace('/api/images/', '').split('?')[0];
                try {
                    const info = await getScreenshotInfo(extractedId);
                    if (info?.contentType) mimeType = info.contentType;
                    const buffer = await getGridFSScreenshot(extractedId);
                    base64 = buffer.toString('base64');
                } catch (err) {
                    console.warn(`[ProductivityQueue] GridFS load from URL path failed:`, err.message);
                    continue;
                }
            } else {
