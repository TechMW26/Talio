...
/**
 * Upload any file to ImageKit directly from buffer or base64.
 * Use this for non-image attachments (pdf/docx/xlsx/zip/etc) as well.
 *
 * @param {Buffer|string} fileData - File buffer, base64 string, or data: URI
 * @param {Object} options
 * @param {string} options.mimeType - MIME type (required unless fileData is a data: URI)
 * @param {string} options.folder - ImageKit folder path
 * @param {string} options.fileName - Filename to use
 * @param {boolean} options.useUniqueFileName - Use unique filename (default: true)
 * @param {Array} options.tags - Tags for the file
 */
export async function uploadFileToImageKit(fileData, options = {}) {
    const imagekit = getImageKit();
    ...
    try {
        ...
        if (typeof fileData === 'string') {
            ...
            if (fileData.startsWith('data:')) {
                ...
                if (!mimeType) {
                    throw new Error('uploadFileToImageKit requires mimeType when fileData is raw base64');
                }
                file = `data:${mimeType};base64,${fileData}`;
            }
        } else {
            if (!mimeType) {
                throw new Error('uploadFileToImageKit requires mimeType when fileData is a Buffer');
            }
            file = `data:${mimeType};base64,${fileData.toString('base64')}`;
        }
