...
import { resolveModelDependencies } from './platform/modelDependencies.js';
import mongoose from 'mongoose';
import bcrypt from 'bcryptjs';
import { compareStoredPassword } from './passwordAuth.js';
import { CANDIDATE_SOURCE_VALUES } from './recruitmentConstants.js';
import { encryptSecret, isEncryptedSecret } from './secretEncryption.js';
import { HRMS_MODULE_KEYS } from './hrms/moduleRegistry.js';
...
const UserSchema = new mongoose.Schema({
  ...
  settings: { screenshotInterval: { type: Number, default: 4 }, screenshotIntervalUpdatedAt: Date },
  ...
  isDepartmentManager: { type: Boolean, default: false },
  departmentManagerOf: [{ type: mongoose.Schema.Types.ObjectId, ref: 'Department' }],
  teamLeaderOf: [{ type: mongoose.Schema.Types.ObjectId, ref: 'Team' }],
  teamMemberOf: [{ type: mongoose.Schema.Types.ObjectId, ref: 'Team' }],
  // ── RBAC fields ──
...
const EmployeeSchema = new mongoose.Schema({
  ...
  status: { type: String, enum: ['active', 'inactive', 'on_leave', 'terminated', 'resigned', 'probation'], default: 'active' },
  dateOfJoining: Date,
  dateOfLeaving: Date,
  employmentType: { type: String, enum: ['full-time', 'part-time', 'contract', 'intern'], default: 'full-time' },
  lifecycle: {
    stage: { type: String, enum: ['preboarding', 'onboarding', 'probation', 'confirmed', 'notice_period', 'offboarding', 'alumni'], default: 'onboarding' },
    onboarding: {
      ...
      checklist: [{
        ...
        verification: {
          ...
          documents: [{ requirementKey: String, label: String, fileName: String, fileUrl: String, fileId: String, fileType: String, fileSize: Number }],
          reopenedAt: Date,
  ...
  assignedTeamLead: { type: mongoose.Schema.Types.ObjectId, ref: 'Employee' },
  ...
  // Historical imports stored a formatted address string while newer profile
  // flows can store structured address parts. Mixed keeps both supported until
  // tenants naturally converge without making unrelated employee edits fail.
  ...
  manualKRIs: [{ type: String, trim: true }],
  manualKPIs: [{
...
EmployeeSchema.index({ assignedManager: 1 });
EmployeeSchema.index({ assignedTeamLead: 1 });
...
const DepartmentSchema = new mongoose.Schema({
  ...
  departmentManager: { type: mongoose.Schema.Types.ObjectId, ref: 'Employee' },
  departmentManagers: [{ type: mongoose.Schema.Types.ObjectId, ref: 'Employee' }],
...
/**
 * Team Schema
 * A Team sits inside a Department. A Department can have multiple Teams.
 * Team leaders can be from ANY department (cross-department leadership).
 */
const TeamSchema = new mongoose.Schema({
  ...
  teamLeaders: [{ type: mongoose.Schema.Types.ObjectId, ref: 'Employee' }],
...
const AttendanceSchema = new mongoose.Schema({
  ...
  location: {
    checkIn: {
      latitude: Number, longitude: Number, address: String, accuracy: Number,
      capturedAt: Date, source: String, warning: String,
      geofenceLocation: { type: mongoose.Schema.Types.ObjectId, ref: 'GeofenceLocation' },
      geofenceLocationName: String,
      addressDetails: { city: String, state: String, country: String, pincode: String, fullAddress: String }
    },
    checkOut: {
      latitude: Number, longitude: Number, address: String, accuracy: Number,
      capturedAt: Date, source: String, warning: String, autoCheckout: Boolean,
      geofenceLocation: { type: mongoose.Schema.Types.ObjectId, ref: 'GeofenceLocation' },
      geofenceLocationName: String,
      addressDetails: { city: String, state: String, country: String, pincode: String, fullAddress: String }
    }
  ...
  source: { type: String, enum: ['manual', 'user_checkin', 'geofence', 'system', 'system_auto_absent', 'system_backfill', 'correction', 'import', 'auto_checkout', 'attendance_machine'], default: 'manual' },
...
/**
 * Attendance machine configuration. Every tenant owns an isolated collection;
 * `scope` optionally narrows a machine to one legal company inside that tenant.
 */
const AttendanceMachineSchema = new mongoose.Schema({
  ...
  serialNumber: { type: String, trim: true, maxlength: 160, default: null },
  scope: { type: String, enum: ['organisation', 'company'], default: 'organisation', required: true },
  company: { type: mongoose.Schema.Types.ObjectId, ref: 'Company', default: null },
  locationName: { type: String, trim: true, maxlength: 200 },
  timezone: { type: String, default: 'Asia/Kolkata' },
  connectionMode: { type: String, enum: ['push_http', 'cloud_api', 'lan_bridge', 'file_import'], required: true },
  endpointUrl: { type: String, trim: true, maxlength: 2000 },
  host: { type: String, trim: true, maxlength: 255 },
  port: { type: Number, min: 1, max: 65535 },
  siteId: { type: String, trim: true, maxlength: 255 },
  credentials: {
  ...
  webhookTokenHash: { type: String, select: false },
  webhookTokenLastFour: String,
  duplicateWindowSeconds: { type: Number, min: 1, max: 3600, default: 30 },
  punchDirectionMode: { type: String, enum: ['device', 'alternate', 'first_last'], default: 'first_last' },
  employeeCodeField: { type: String, default: 'employeeCode', trim: true, maxlength: 100 },
  status: { type: String, enum: ['active', 'disabled', 'error'], default: 'active' },
  lastSeenAt: Date,
  lastSyncAt: Date,
  lastError: { type: String, maxlength: 1000 },
  syncCursor: { type: String, maxlength: 1000 },
  createdBy: { type: mongoose.Schema.Types.ObjectId, ref: 'User', required: true },
  updatedBy: { type: mongoose.Schema.Types.ObjectId, ref: 'User', required: true },
}, { timestamps: true, strict: true });
AttendanceMachineSchema.index({ scope: 1, company: 1, status: 1 });
AttendanceMachineSchema.index(
  { providerKey: 1, serialNumber: 1 },
  { unique: true, partialFilterExpression: { serialNumber: { $type: 'string' } } }
);

/** Immutable, idempotent audit trail of raw device punches. */
const AttendanceMachinePunchSchema = new mongoose.Schema({
  machine: { type: mongoose.Schema.Types.ObjectId, ref: 'AttendanceMachine', required: true },
  ...
  processingStatus: { type: String, enum: ['processed', 'unmapped', 'rejected'], default: 'processed' },
  processingError: String,
  rawPayload: mongoose.Schema.Types.Mixed,
}, { timestamps: true, strict: true });
AttendanceMachinePunchSchema.index({ machine: 1, eventKey: 1 }, { unique: true });
AttendanceMachinePunchSchema.index({ employee: 1, punchedAt: -1 });
AttendanceMachinePunchSchema.index({ processingStatus: 1, createdAt: -1 });
...
const LeaveSchema = new mongoose.Schema({
  ...
  requestType: { type: String, enum: ['leave', 'half_day', 'work_from_home', 'early_leave'], default: 'leave' },
  earlyLeaveTime: String,
  emergencyContact: String,
  handoverNotes: String,
  attachments: [{ url: String, fileId: String, fileName: String }],
}, { timestamps: true, strict: false });
LeaveSchema.pre('validate', function syncLeaveDayFields(next) {
...
const LeaveTypeSchema = new mongoose.Schema({
  ...
  minDaysNotice: { type: Number },
}, { timestamps: true, strict: false });
LeaveTypeSchema.pre('validate', function syncLeaveTypeFields(next) {
  const daysPerYear = Number(this.maxDaysPerYear ?? this.daysPerYear ?? 0)
  this.maxDaysPerYear = Number.isFinite(daysPerYear) ? daysPerYear : 0
...
LeaveBalanceSchema.pre('validate', function syncLeaveBalanceFields(next) {
  ...
  const storedRemaining = this.remainingDays ?? this.balance
  const remainingDays = storedRemaining === undefined || storedRemaining === null
...
const HolidaySchema = new mongoose.Schema({
  ...
  dayPortion: { type: String, enum: ['full_day', 'first_half', 'second_half'], default: 'full_day' },
  isActive: { type: Boolean, default: true },
...
const CompanySettingsSchema = new mongoose.Schema({
  ...
  leave: {
    ...
    halfDayPolicy: {
      defaultAnnualLimit: { type: Number, default: 12, min: 0 },
      limitsByLevel: [{
  ...
  integrations: {
    linkedin: {
      ...
      connectedAccountName: { type: String, trim: true },
      connectedAccountId: { type: String, trim: true },
...
CompanySettingsSchema.pre('save', function companySettingsEncryptLinkedInTokens(next) {
  const linkedinSettings = this.integrations?.linkedin;
  ...
  if (linkedinSettings.accessToken && !isEncryptedSecret(linkedinSettings.accessToken)) {
    linkedinSettings.accessToken = encryptSecret(linkedinSettings.accessToken);
  }

  if (linkedinSettings.refreshToken && !isEncryptedSecret(linkedinSettings.refreshToken)) {
    linkedinSettings.refreshToken = encryptSecret(linkedinSettings.refreshToken);
  }
...
const GeofenceLocationSchema = new mongoose.Schema({
  ...
  strictMode: { type: Boolean, default: false },
  scope: { type: String, enum: ['organisation', 'company'], default: 'organisation' },
  company: { type: mongoose.Schema.Types.ObjectId, ref: 'Company', default: null },
  ...
  address: String,
  workingHours: { checkInTime: String, checkOutTime: String, workingDays: [String] },
  breakTimings: [{ name: String, startTime: String, endTime: String, days: [String], isActive: { type: Boolean, default: true } }],
  createdBy: { type: mongoose.Schema.Types.ObjectId, ref: 'Employee' },
  updatedBy: { type: mongoose.Schema.Types.ObjectId, ref: 'Employee' },
  stats: { totalCheckIns: { type: Number, default: 0 }, lastCheckInAt: Date },
}, { timestamps: true, strict: false });
...
const GeofenceLogSchema = new mongoose.Schema({
  employee: { type: mongoose.Schema.Types.ObjectId, ref: 'Employee', required: true },
  user: { type: mongoose.Schema.Types.ObjectId, ref: 'User', required: true },
  eventType: { type: String, enum: ['exit', 'entry', 'outside_during_hours', 'location_update', 'attendance_check_in', 'attendance_check_out'], required: true },
  ...
  geofenceLocation: { type: mongoose.Schema.Types.ObjectId, ref: 'GeofenceLocation' },
  geofenceLocationName: String,
  checkedLocations: [{ locationId: { type: mongoose.Schema.Types.ObjectId, ref: 'GeofenceLocation' }, locationName: String, distance: Number, isWithin: Boolean }],
  duringBreakTime: { type: Boolean, default: false },
  breakTimingName: String,
  outOfPremisesRequest: {
    reason: String, requestedAt: Date,
    status: { type: String, enum: ['pending', 'approved', 'rejected'], default: 'pending' },
    reviewedBy: { type: mongoose.Schema.Types.ObjectId, ref: 'Employee' },
    reviewedAt: Date, reviewerComments: String,
  },
  department: { type: mongoose.Schema.Types.ObjectId, ref: 'Department' },
  reportingManager: { type: mongoose.Schema.Types.ObjectId, ref: 'Employee' },
  duringWorkHours: { type: Boolean, default: false },
...
AttendanceCorrectionSchema.index({ status: 1, employee: 1 });
...
// Recent-list queries used by the dashboard and MIRA.
...
const ProjectMemberSchema = new mongoose.Schema({
  ...
  user: { type: mongoose.Schema.Types.ObjectId, ref: 'Employee', required: true },
  role: { type: String, enum: ['head', 'member', 'observer', 'external', 'team_leader'], default: 'member' },
...
ExpenseSchema.index({ status: 1, employee: 1 });
...
const AssetSchema = new mongoose.Schema({
  ...
  category: String,
  description: String,
  status: { type: String, enum: ['available', 'assigned', 'under-maintenance', 'damaged', 'disposed', 'maintenance', 'retired'], default: 'available' },
...
const PolicySchema = new mongoose.Schema({
  ...
  applicableTo: { type: String, enum: ['all', 'department', 'company', 'specific'], default: 'all' },
...
const JobPostingSchema = new mongoose.Schema({
  ...
  linkedinJobPostingId: { type: String, trim: true },
  linkedinPostedAt: { type: Date },
  linkedinPostingUrl: { type: String, trim: true },
  hiringPipeline: [{
...
const CandidateSchema = new mongoose.Schema({
  ...
  source: { type: String, enum: CANDIDATE_SOURCE_VALUES, default: 'website' },
  sourceUrl: { type: String, trim: true },
  sourceMetadata: { type: mongoose.Schema.Types.Mixed },
  linkedinProfileUrl: { type: String, trim: true },
  linkedinPublicIdentifier: { type: String, trim: true },
  linkedinHeadline: { type: String, trim: true },
  linkedinCurrentCompany: { type: String, trim: true },
  linkedinCurrentPosition: { type: String, trim: true },
  linkedinSkills: [{ type: String, trim: true }],
  linkedinImportedAt: { type: Date },
...
const ScreenshotSchema = new mongoose.Schema({
  ...
  // Daily-analysis tracking (replaces session-based grouping)
  analyzed: {
    type: Boolean,
    default: false,
    index: true
...
ScreenshotSchema.index({ user: 1, dateString: 1, analyzed: 1 });
...
const ProductivitySessionSchema = new mongoose.Schema({
  ...
  // Desktop-side session identifier used to keep server grouping exact.
  sourceSessionId: {
    type: String,
    index: true,
    default: null
...
/**
 * ProbationApproval Schema
 * Durable approval record for confirmation/extension decisions.
 */
const ProbationApprovalSchema = new mongoose.Schema({
  ...
  approverSource: { type: String, enum: ['reportingManager', 'assignedTeamLead', 'assignedManager', 'reportsTo'], required: true },
...
const ActionableNotificationSchema = new mongoose.Schema({
  ...
  type: {
    type: String,
    enum: ['project_invitation', 'task_assignment', 'meeting_invitation', 'leave_approval', 'expense_approval', 'document_approval', 'travel_approval', 'attendance_correction', 'helpdesk_assignment', 'announcement', 'probation_approval', 'generic'],
    required: true,
    index: true
  },
  priority: { type: String, enum: ['low', 'medium', 'high', 'urgent'], default: 'medium' },
  ...
  reference: {
    model: { type: String, enum: ['Project', 'Task', 'Meeting', 'Leave', 'Expense', 'Travel', 'Document', 'Attendance', 'Helpdesk', 'Announcement', 'ProbationApproval'] },
...
/**
 * ScreenshotComposite Schema
 * One stitched mosaic per (user, dateString). After AI analysis succeeds we
 * compose all analyzed screenshots into a single image, store it in GridFS,
 * and delete the individual Screenshot docs + their GridFS files. Subsequent
 * batches re-stitch the existing composite with the new screenshots so the
 * single image grows monotonically. The `tiles` array maps every original
 * screenshot to its rectangle inside the composite so the UI can zoom in.
 */
const ScreenshotCompositeSchema = new mongoose.Schema({
  user: { type: mongoose.Schema.Types.ObjectId, ref: 'User', required: true, index: true },
  ...
  tiles: [{
    ...
    height: Number,
    capturedAt: Date,
    originalScreenshotId: mongoose.Schema.Types.ObjectId,
    activity: String,
    productivity: String,
    applicationVisible: String,
    websiteVisible: String,
  ...
  lastStitchedAt: { type: Date, default: Date.now },
  completedAt: Date,
  expiresAt: Date,
  source: String,
}, { timestamps: true, strict: false });
ScreenshotCompositeSchema.index({ user: 1, dateString: 1 }, { unique: true });
ScreenshotCompositeSchema.index({ employee: 1, date: -1 });
ScreenshotCompositeSchema.index({ expiresAt: 1 });
...
/**
 * ProjectEmailNotificationLog Schema
 * Stores one delivery record per recipient for project/task event emails.
 */
const ProjectEmailNotificationLogSchema = new mongoose.Schema({
  triggerType: {
    type: String,
    enum: ['project_created', 'task_created', 'project_status_changed', 'task_status_changed'],
    required: true
  },
  triggerKey: {
    type: String,
  ...
  attachments: [{
    ...
    status: {
      type: String,
      enum: ['attached', 'missing', 'skipped'],
      default: 'skipped'
    },
    reason: String
  }]
}, { timestamps: true, strict: false });
ProjectEmailNotificationLogSchema.index({ triggerKey: 1 }, { unique: true });
ProjectEmailNotificationLogSchema.index({ project: 1, createdAt: -1 });
ProjectEmailNotificationLogSchema.index({ task: 1, createdAt: -1 });
ProjectEmailNotificationLogSchema.index({ recipientEmail: 1, createdAt: -1 });
ProjectEmailNotificationLogSchema.index({ status: 1, createdAt: -1 });
ProjectEmailNotificationLogSchema.index({ queued: 1, scheduledFor: 1 });
ProjectEmailNotificationLogSchema.index({ status: 1, queued: 1, scheduledFor: 1 });
...
const WebhookDeliveryLogSchema = new mongoose.Schema({
  // Stable across managed-queue retries; makes at-least-once delivery idempotent.
...
WebhookDeliveryLogSchema.index({ deliveryId: 1 }, { unique: true, sparse: true });
...
/**
 * ForceRefresh Schema
 * Stores pending force-refresh commands for users when Socket.IO is unavailable.
 * Documents auto-delete after 2 minutes via TTL index.
 */
const ForceRefreshSchema = new mongoose.Schema({
  userId: { type: mongoose.Schema.Types.ObjectId, required: true, index: true },
  message: { type: String, default: 'The administrator requested a background data sync.' },
...
ForceRefreshSchema.index({ userId: 1, consumed: 1, createdAt: -1 });
ForceRefreshSchema.index({ createdAt: 1 }, { expireAfterSeconds: 120 });

/**
 * TicTacToeGame Schema - DB-backed game state for TicTacToe (works without Socket.IO)
 * TTL index auto-deletes games after 1 hour.
 */
...
TicTacToeGameSchema.index({ hostUserId: 1, status: 1 });
TicTacToeGameSchema.index({ guestUserId: 1, status: 1 });
TicTacToeGameSchema.index({ createdAt: 1 }, { expireAfterSeconds: 3600 });

/**
 * UserPresence Schema - DB-backed heartbeat for presence tracking
 * Clients POST to /api/user/heartbeat every 60s. Users with lastHeartbeat within
 * the last 2 minutes are considered "active now".
 * TTL index auto-deletes stale documents after 5 minutes.
 */
...
UserPresenceSchema.index({ lastHeartbeat: 1 }, { expireAfterSeconds: 300 });

/**
 * MiraTokenUsage Schema - Track monthly AI chat token usage per user
 * Each user gets 100 tokens/month. 1 message = 1 token.
 */
const MiraTokenUsageSchema = new mongoose.Schema({
  user: { type: mongoose.Schema.Types.ObjectId, ref: 'User', required: true },
  month: { type: String, required: true }, // "2026-03" format
  tokensUsed: { type: Number, default: 0 },
  tokenLimit: { type: Number, default: 100 },
}, { timestamps: true });
MiraTokenUsageSchema.index({ user: 1, month: 1 }, { unique: true });

/**
 * MiraChatSession Schema - stores MIRA chat sessions per user
 */
const MiraChatSessionSchema = new mongoose.Schema({
  user: { type: mongoose.Schema.Types.ObjectId, ref: 'User', required: true },
  title: { type: String, default: 'New Chat', trim: true },
  messages: [{
    role: { type: String, enum: ['user', 'assistant'], required: true },
    content: { type: String, required: true },
    data: { type: mongoose.Schema.Types.Mixed }, // AI response data (cards, suggestions)
    timestamp: { type: Date, default: Date.now }
  }],
  lastMessageAt: { type: Date, default: Date.now },
}, { timestamps: true });
MiraChatSessionSchema.index({ user: 1, lastMessageAt: -1 });

const MiraGeneratedImageSchema = new mongoose.Schema({
  user: { type: mongoose.Schema.Types.ObjectId, ref: 'User', required: true },
...
MiraGeneratedImageSchema.index({ user: 1, requestId: 1 }, { unique: true });

/**
 * Role Schema — RBAC role definition per company/tenant
 * Each role has a unique name within a company and a permissions object
 * shaped exactly as PERMISSIONS_SCHEMA from lib/permissions.js.
 */
...
/**
 * RBACAuditLog Schema — tracks all RBAC-related events
 * TTL index auto-deletes entries after 90 days.
 */
const RBACAuditLogSchema = new mongoose.Schema({
  eventType: {
    type: String,
    enum: ['role_created', 'role_updated', 'role_deleted', 'user_role_changed', 'permission_denied'],
    required: true,
  },
  actorId: { type: mongoose.Schema.Types.ObjectId, ref: 'User' },
  targetId: { type: mongoose.Schema.Types.ObjectId },
  targetType: { type: String, enum: ['Role', 'User'] },
  metadata: { type: mongoose.Schema.Types.Mixed, default: {} },
  ipAddress: { type: String },
  userAgent: { type: String },
  createdAt: { type: Date, default: Date.now },
});
RBACAuditLogSchema.index({ createdAt: 1 }, { expireAfterSeconds: 90 * 24 * 60 * 60 });
RBACAuditLogSchema.index({ eventType: 1, createdAt: -1 });
RBACAuditLogSchema.index({ actorId: 1, createdAt: -1 });

/**
 * Unified HRMS workflow case. Missing and partial HRMS modules share this
 * lifecycle primitive instead of implementing incompatible approval tables.
 */
const HrmsWorkflowSchema = new mongoose.Schema({
  caseNumber: { type: String, required: true, unique: true, trim: true },
  module: { type: String, enum: HRMS_MODULE_KEYS, required: true, index: true },
  title: { type: String, required: true, trim: true, maxlength: 200 },
  ...
  subjectEmployee: { type: mongoose.Schema.Types.ObjectId, ref: 'Employee', default: null },
  owner: { type: mongoose.Schema.Types.ObjectId, ref: 'User', required: true },
  ...
  source: {
    module: String,
    caseId: { type: mongoose.Schema.Types.ObjectId, ref: 'HrmsWorkflow' },
    entityType: String,
    entityId: mongoose.Schema.Types.ObjectId,
  },
  linkedCases: [{ type: mongoose.Schema.Types.ObjectId, ref: 'HrmsWorkflow' }],
  approvals: [{
...
HrmsWorkflowSchema.index({ module: 1, status: 1, createdAt: -1 });
HrmsWorkflowSchema.index({ subjectEmployee: 1, createdAt: -1 });
HrmsWorkflowSchema.index({ owner: 1, status: 1, createdAt: -1 });
HrmsWorkflowSchema.index({ idempotencyKey: 1 }, { unique: true, sparse: true });

const HrmsWorkflowEventSchema = new mongoose.Schema({
  workflow: { type: mongoose.Schema.Types.ObjectId, ref: 'HrmsWorkflow', required: true },
  module: { type: String, enum: HRMS_MODULE_KEYS, required: true },
...
HrmsWorkflowEventSchema.index({ workflow: 1, createdAt: -1 });
HrmsWorkflowEventSchema.index({ module: 1, createdAt: -1 });
...
const SCHEMAS = {
  ...
  AttendanceMachine: AttendanceMachineSchema,
  AttendanceMachinePunch: AttendanceMachinePunchSchema,
  Leave: LeaveSchema,
  ...
  ProbationApproval: ProbationApprovalSchema,
  ...
  TicTacToeGame: TicTacToeGameSchema,
  MiraTokenUsage: MiraTokenUsageSchema,
  MiraChatSession: MiraChatSessionSchema,
  MiraGeneratedImage: MiraGeneratedImageSchema,
  Role: RoleSchema,
  RBACAuditLog: RBACAuditLogSchema,
  HrmsWorkflow: HrmsWorkflowSchema,
  HrmsWorkflowEvent: HrmsWorkflowEventSchema,
};
...
const MODEL_DEPENDENCIES = {
  ...
  'User': ['Employee', 'Department', 'Company', 'Team'],
  'Team': ['Department', 'Employee'],
  'Attendance': ['Employee', 'Department', 'Company'],
  'AttendanceMachine': ['Company', 'User'],
  'AttendanceMachinePunch': ['AttendanceMachine', 'Employee', 'Company'],
  'Leave': ['Employee', 'LeaveType', 'Department'],
  'LeaveBalance': ['Employee', 'LeaveType'],
  'Project': ['Employee', 'Department', 'Team'],
  ...
  'ProbationApproval': ['Employee', 'User'],
...
export async function getTenantModel(databaseName, modelName) {
  ...
  // Resolve aliases
  const resolvedName = Object.hasOwn(SCHEMA_ALIASES, modelName) ? SCHEMA_ALIASES[modelName] : modelName;

  if (typeof modelName !== 'string' || !Object.hasOwn(SCHEMAS, resolvedName)) {
  ...
  if (dbCache.has(resolvedName)) {
    ...
    if (cachedModel?.db?.readyState === 1 && cachedModel.db.models[resolvedName] === cachedModel) {
      return cachedModel;
    }
  ...
  try {
    ...
    if (pendingModels.get(pendingKey) === modelPromise) pendingModels.delete(pendingKey);
  }
}
...
async function createModel(databaseName, resolvedName, dbCache) {
  ...
  return compileModel(connection, resolvedName, dbCache);
}

function compileModel(connection, resolvedName, dbCache) {
  ...
  if (connection.models[resolvedName]) {
    const existingModel = connection.models[resolvedName];
    const needsUserModelRepair = resolvedName === 'User' && typeof existingModel?.prototype?.comparePassword !== 'function';

    if (needsUserModelRepair) {
      connection.deleteModel(resolvedName);
 ...
 * Returns the same transitive dependency set on cold and warm requests.
 * Existing connection models are reused without recompilation.
 * 
 * @param {string} databaseName - The tenant's database name
...
export async function getTenantModels(databaseName, modelNames) {
  const resolved = resolveModelDependencies(modelNames, SCHEMAS, SCHEMA_ALIASES, MODEL_DEPENDENCIES);
  const connection = await getTenantConnection(databaseName);
  if (!modelCache.has(databaseName)) modelCache.set(databaseName, new Map());
  const dbCache = modelCache.get(databaseName);
  const models = {};
  // One connection lookup and synchronous compilation. connection.models is the
  // authority; dependency visibility must not depend on which request ran first.
  for (const name of resolved.names) {
    models[name] = compileModel(connection, name, dbCache);
  }
  for (const [alias, canonical] of resolved.aliases) models[alias] = models[canonical];
  return models;
}
...
export default tenantModelsApi;
