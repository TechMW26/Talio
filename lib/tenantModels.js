...
async function createModel(databaseName, resolvedName, dbCache) {
  ...
  // Always use fresh schema by removing existing model first
  let model;
  try {
 ...
 * Automatically loads dependent models required for populate() to work
 * IMPORTANT: Dependencies are loaded FIRST to ensure populate() works correctly
 * @param {string} databaseName - The tenant's database name
...
export async function getTenantModels(databaseName, modelNames) {
  ...
  // Collect all models including dependencies
  ...
  for (const name of modelNames) {
    ...
    if (deps) {
      deps.forEach(dep => allModelsToLoad.add(dep));
    }
  }

  // Add the requested models AFTER dependencies
  ...
  // Load all models (dependencies first, then requested models)
  for (const name of allModelsToLoad) {
