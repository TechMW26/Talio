...
import { SCREENSHOT_RETENTION_HOURS, getScreenshotRetentionCutoff } from '@/lib/productivitySessionRules';
...
/**
 * Run screenshot cleanup - delete screenshots older than the active retention window.
 */
export async function runScreenshotCleanup() {
  ...
  try {
    ...
    console.log(`[Cleanup] Found ${screenshotsToDelete.length} screenshots older than ${SCREENSHOT_RETENTION_HOURS} hours`);
    ...
    // Delete from GridFS
    const gridfsResult = await deleteOldScreenshots(cutoff);
...
- Designation/Job Title: ${employeeContext?.designation || 'Not specified'}
${!employeeContext?.designation && employeeContext?.department ? `- Department: ${employeeContext.department} (fallback - no specific job title available)` : ''}
- Expected Workflow: ${employeeContext?.expectedWorkflow || 'General office work'}

IMPORTANT: Use the employee's DESIGNATION to determine if the activity is appropriate for their role.
The designation tells us WHAT they should be doing (e.g., "Developer" = coding, "Designer" = designing).
...
- Name: ${employeeContext?.name || 'Unknown'}
- Designation/Job Title: ${employeeContext?.designation || 'Not specified'}
${!employeeContext?.designation && employeeContext?.department ? `- Department: ${employeeContext.department} (fallback context)` : ''}
- Expected Workflow: ${employeeContext?.expectedWorkflow || 'General office work'}

IMPORTANT: Judge productivity based on the employee's DESIGNATION (job title), not their department.
A "Software Developer" should be coding; a "Designer" should be designing.
...
Generate a summary in JSON format:
{
  ...
  "concerns": ["Concern if any, especially if activities don't match expected workflow for their designation", ...],
 ...
 * STRICT scoring: entertainment and idle have significant penalties
 */
function calculateMetrics(analyses) {
  ...
  // STRICTER productivity scores - high bar for high productivity
  const productivityScores = {
    high: 95,      // Only active, focused work
    medium: 60,    // Some work but not fully focused
    low: 30,       // Mostly distracted
    idle: 5        // No meaningful activity
  };

  // STRICTER category scores - entertainment is penalized heavily
  const categoryScores = {
    work: 95,           // Active coding, writing, designing
    research: 80,       // Reading documentation, learning (must be active)
    meeting: 75,        // Collaboration time
    communication: 60,  // Slack, email - can be productive or distraction
    other: 40,          // Unknown activities - assume low value
    break: 20,          // Acceptable but not productive
    entertainment: 10,  // YouTube, social media, gaming
    idle: 5             // Screen unchanged or locked
  };
  ...
  for (const analysis of analyses) {
    productivitySum += productivityScores[analysis.productivity] || 40;
    categorySum += categoryScores[analysis.activity] || 40;

    if (analysis.activity === 'entertainment') entertainmentCount++;
    if (analysis.productivity === 'idle' || analysis.activity === 'idle') idleCount++;
  }

  const avgProductivity = productivitySum / analyses.length;
  const avgCategory = categorySum / analyses.length;

  // Apply penalties for entertainment and idle time
  const entertainmentPenalty = (entertainmentCount / analyses.length) * 30; // Up to -30 for all entertainment
  const idlePenalty = (idleCount / analyses.length) * 20; // Up to -20 for all idle
  ...
  const scores = analyses.map(a => productivityScores[a.productivity] || 40);
  const mean = scores.reduce((a, b) => a + b, 0) / scores.length;
  const variance = scores.reduce((a, b) => a + Math.pow(b - mean, 2), 0) / scores.length;
  const consistencyScore = Math.max(0, 100 - Math.sqrt(variance));

  // Calculate overall score with penalties applied
  const rawOverall = (avgProductivity + avgCategory) / 2;
  const overallScore = Math.max(0, Math.round(rawOverall - entertainmentPenalty - idlePenalty));
