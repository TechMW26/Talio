 ...
 * @throws {Error} - Throws on connection errors (don't mask as "user not found")
 */
export async function getTenantByEmail(email) {
  ...
  try {
    // Ensure superadmin connection - this may throw on network errors
    ...
    // Re-throw connection errors - don't mask them as "user not found"
    // This allows the login route to show appropriate error messages
    throw error;
  }
}

/**
 * Get tenant info by user ID (MongoDB _id)
 * @param {string} userId - User's _id from the tenant database
...
export async function getTenantByUserId(userId) {
  ...
  try {
    ...
    console.error('[TenantContext] Error looking up tenant for userId:', error.message);
    return null;
  }
}
