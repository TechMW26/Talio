...
function startDailyProductivityCron() {
    ...
    // Default: every 15 minutes. The cron route inspects each tenant's
    // local time and only acts on those whose local clock just crossed
    // midnight (within the configured window). Override with
    // DAILY_PRODUCTIVITY_CRON.
    const cronExpression = process.env.DAILY_PRODUCTIVITY_CRON || '*/15 * * * *';
