...
export async function prepareMiraAction(action, user, models) {
  ...
  if (type === 'invite_project' || type === 'invite_meeting') {
    const target = await prepareMiraAction({ type: type === 'invite_project' ? 'open_project' : 'open_meeting', fields: { query: f.query } }, user, models)
    const ids = []
    for (const name of f.invitees) ids.push(await resolveMiraPerson(name, user, models, { field: 'invitees', type }))
