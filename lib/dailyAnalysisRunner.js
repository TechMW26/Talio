...
export async function runDailyAnalysis({ userId, dateString, models, tenant, trigger = 'manual', forceReanalyze = false }) {
  ...
  const mismatched = pendingScreenshots.find((s) => {
    ...
    const screenshotDate = new Date(s.capturedAt).toISOString().split('T')[0];
  ...
  if (pendingScreenshots.length > 0) {
    ...
    const { stitchedIds } = await appendScreenshotsToComposite({
  ...
  const { composite, buffer: compositeBuffer } = await getCompositeImageBuffer({
    models,
    tenant,
    userId: targetUserId,
    dateString,
