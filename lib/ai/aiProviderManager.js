...
function isFatalRequestError(error, providerName) {
    if (!error) return false;
    if (error.errorClass === 'client' && providerName === 'gemini') return true;
    return false;
}

function shouldFallOver(error, providerName) {
    ...
    if (providerName === 'custom' || providerName === 'inference') return true;
...
function buildProviderChain(kind) {
    ...
    if (kind === 'rawVision' && envEnabled(process.env.CUSTOM_AI_ANALYSIS_ONLY || process.env.AI_ANALYSIS_CUSTOM_ONLY)) {
        return [chain[0]];
    }
...
async function routeRequest(kind, args) {
    ...
    for (let index = 0; index < configuredProviders.length; index += 1) {
        ...
        if (!provider.alwaysAttempt && !healthMonitor.isAvailable(provider.name) && !isLastConfiguredProvider) {
        ...
        try {
            ...
            if (isFatalRequestError(error, provider.name)) {
                throw error;
            }
            if (!shouldFallOver(error, provider.name)) {
                throw error;
            }
