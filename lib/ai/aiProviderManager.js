// DeepSeek handles text; Pollinations handles vision, without cross-fallback.
// Public surface is identical to the original
...
import { generateDeepSeekContent, streamDeepSeekContent, getDeepSeekAvailability } from './providers/deepseekProvider.js';

const MAX_RETRIES = 3;

/**
 * Wrap a non-streaming provider call with retry logic. On rate-limit (429) or server
 * errors we retry up to MAX_RETRIES times with exponential backoff and jitter.
 */
async function withRetry(fn, opLabel, maxAttempts = MAX_RETRIES) {
    maxAttempts = Math.max(1, Math.min(MAX_RETRIES, Number(maxAttempts) || MAX_RETRIES));
    let lastError = null;
    for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
        try {
            const startedAt = Date.now();
            const result = await fn();
            aiLogger.info('ai.success', {
                op: opLabel,
                attempt,
            ...
            aiLogger.warn('ai.attempt_failed', {
                op: opLabel,
                attempt,
    ...
    const err = new Error(
        `AI call failed after at most ${maxAttempts} attempts: ${lastError?.message || 'unknown error'}`
    );
...
export async function generateContent(prompt, systemInstruction = '', options = {}) {
    return withRetry(
        () => generateDeepSeekContent(prompt, systemInstruction, options),
        'text', options.maxAttempts
    );
}
...
export function getAIAvailability() {
    ...
    const deepseek = getDeepSeekAvailability();

    return {
        // Backward-compatible fields.
        anyAvailable: deepseek.configured,
        textAvailable: deepseek.configured,
        visionAvailable: pollinations.configured,
        deepseekConfigured: deepseek.configured,
        pollinationsConfigured: pollinations.configured,
        // Extended diagnostics.
        providers: { deepseek, pollinations },
        provider: 'deepseek',
    };
}

export function _resetRouterHealth() {
    // No key rotation state to reset.
}

export const streamContent = streamDeepSeekContent;
