...
export class KeyRotationManager {
    ...
    *iterateHealthy() {
    ...
    /**
     * Mark a key as failed. If `retryAfterMs` is provided (from a 429
     * Retry-After header), use that exact value instead of exponential
     * backoff so we honour Google's rate-limit window precisely.
     */
    markFailure(index, errorClass = 'unknown', retryAfterMs = null) {
        if (index < 0 || index >= this.health.length) return;
        const h = this.health[index];
        h.failures += 1;
        h.lastErrorClass = errorClass;
        if (retryAfterMs && retryAfterMs > 0) {
            // Honouring Retry-After header directly
            h.cooldownUntil = Date.now() + retryAfterMs;
        } else {
