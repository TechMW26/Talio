...
const SERVICE_PROFILES = {
    custom: {
        ...
        chatApiKey: () => process.env.CUSTOM_AI_CHAT_API_KEY || process.env.CUSTOM_AI_API_KEY,
        apiKeyHeader: () => process.env.CUSTOM_AI_API_KEY_HEADER,
        apiUrl: () => process.env.CUSTOM_AI_API_URL || process.env.CUSTOM_AI_CHAT_URL,
        requestFormat: () => process.env.CUSTOM_AI_REQUEST_FORMAT || process.env.CUSTOM_AI_PROTOCOL,
        imageFormat: () => process.env.CUSTOM_AI_CHAT_IMAGE_FORMAT || process.env.CUSTOM_AI_IMAGE_FORMAT,
        model: () => process.env.CUSTOM_AI_MODEL || DEFAULT_CHAT_MODEL,
        stream: () => process.env.CUSTOM_AI_STREAM,
    ...
    inference: {
        ...
        chatApiKey: () => process.env.INFERENCE_CHAT_API_KEY || process.env.INFERENCE_API_KEY,
        apiKeyHeader: () => process.env.INFERENCE_API_KEY_HEADER,
        apiUrl: () => process.env.INFERENCE_API_URL || process.env.INFERENCE_CHAT_URL,
        requestFormat: () => process.env.INFERENCE_REQUEST_FORMAT || process.env.INFERENCE_PROTOCOL,
        imageFormat: () => process.env.INFERENCE_CHAT_IMAGE_FORMAT || process.env.INFERENCE_IMAGE_FORMAT,
        model: () => process.env.INFERENCE_MODEL || DEFAULT_CHAT_MODEL,
        stream: () => process.env.INFERENCE_STREAM,
        maxTokens: () => process.env.INFERENCE_MAX_TOKENS,
...
function normalizeRequestFormat(value, apiUrl = '') {
    const normalized = `${value || ''}`.trim().toLowerCase();
    if (['chat', 'openai', 'openai-chat', 'salad', 'salad-chat'].includes(normalized)) return 'chat';
    if (apiUrl) return 'chat';
    return 'legacy';
}

function normalizeChatImageFormat(value, url = '') {
    const normalized = `${value || ''}`.trim().toLowerCase();
    if (['ollama', 'salad', 'message-images', 'images'].includes(normalized)) return 'ollama';
    if (['openai', 'openai-content', 'content-array'].includes(normalized)) return 'openai';
    if (`${url}`.includes('salad.cloud')) return 'ollama';
    return 'openai';
}

function resolveServiceUrl(baseUrl, pathOrUrl) {
    if (/^https?:\/\//i.test(`${pathOrUrl || ''}`)) return pathOrUrl.replace(/\/$/, '');
    return `${baseUrl}${pathOrUrl || ''}`;
}

function getChatHeaderName(profile, url) {
    if (profile.apiKeyHeader()) return profile.apiKeyHeader();
    if (`${url}`.includes('salad.cloud')) return 'Salad-Api-Key';
    return 'Authorization';
}

function buildServiceConfig(profile, serviceKey, baseUrl, mode, headerName, token, path, requestFormat = 'legacy') {
    if (!token) return null;
    return {
        url: resolveServiceUrl(baseUrl, path),
        headerName,
        token,
        mode,
        label: profile.label,
...
function buildChatServiceConfig(profile, serviceKey, baseUrl) {
    const url = resolveServiceUrl(baseUrl, profile.apiUrl() || profile.protectedPath());
    if (!url) return null;

    const token = profile.chatApiKey() || profile.appToken();
    if (!token) return null;

    return {
        url,
        headerName: getChatHeaderName(profile, url),
        token,
        mode: profile.chatApiKey() ? 'protected' : 'public',
        label: profile.label,
        serviceKey,
        requestFormat: 'chat',
        imageFormat: normalizeChatImageFormat(profile.imageFormat(), url),
        model: profile.model(),
        stream: parseBoolean(profile.stream(), false),
        maxTokens: parsePositiveInteger(profile.maxTokens(), DEFAULT_CHAT_MAX_TOKENS),
    };
}
...
function getServiceConfigs(serviceKey) {
    ...
    const requestFormat = normalizeRequestFormat(profile.requestFormat(), apiUrl);
    if (requestFormat === 'chat') {
        return [buildChatServiceConfig(profile, serviceKey, baseUrl)].filter(Boolean);
    }
...
function extractChatResult(data) {
    ...
    return normalizeChatContent(choice?.delta?.content)
        || normalizeChatContent(choice?.message?.content)
        || normalizeChatContent(choice?.text)
        || normalizeChatContent(data?.message?.content)
        || normalizeChatContent(data?.response)
        || normalizeChatContent(data?.result)
        || normalizeChatContent(data?.content);
}

function getChatResult(label, rawText) {
    ...
    if (text.includes('data:')) {
        ...
        for (const line of text.split(/\r?\n/)) {
            ...
            try {
                chunks.push(extractChatResult(JSON.parse(payload)));
            } catch {
                // Ignore malformed stream keep-alives and keep collecting chunks.
            }
        }
    ...
    if (jsonLines.length > 1) {
        ...
        for (const line of jsonLines) {
            try {
                chunks.push(extractChatResult(JSON.parse(line)));
            } catch {
                chunks.length = 0;
                break;
            }
    ...
    try {
        ...
        const result = extractChatResult(parsed).trim();
        if (result) return result;
    } catch {
        // Some lightweight proxies return plain text even for chat endpoints.
        return text;
    }
...
function buildChatUserMessage(prompt, upload = null, serviceConfig) {
    if (!upload) return { role: 'user', content: prompt };

    const imageBase64 = Buffer.from(upload.buffer).toString('base64');

    if (serviceConfig.imageFormat === 'ollama') {
        return {
            role: 'user',
            content: prompt,
            images: [imageBase64],
    ...
    return {
        ...
        content: [
            { type: 'text', text: prompt },
            {
                type: 'image_url',
                image_url: {
                    url: `data:${upload.mimeType};base64,${imageBase64}`,
                },
            },
        ],
    };
...
function buildChatRequestBody(serviceConfig, prompt, upload = null, { systemInstruction = '' } = {}) {
    const messages = [];
    if (systemInstruction) {
        messages.push({ role: 'system', content: systemInstruction });
    }
    messages.push(buildChatUserMessage(prompt, upload, serviceConfig));

    return {
        model: serviceConfig.model,
        messages,
        stream: serviceConfig.stream,
        max_tokens: serviceConfig.maxTokens,
    };
}

function buildServiceHeaders(serviceConfig, extraHeaders = {}) {
    const headerName = serviceConfig.headerName;
    const token = headerName.toLowerCase() === 'authorization' && !/^bearer\s+/i.test(serviceConfig.token)
        ? `Bearer ${serviceConfig.token}`
        : serviceConfig.token;
...
async function executeSingleServiceRequest(serviceConfig, prompt, upload = null, options = {}) {
    const isChatRequest = serviceConfig.requestFormat === 'chat';
    const body = isChatRequest
        ? JSON.stringify(buildChatRequestBody(serviceConfig, prompt, upload, options))
        : buildServiceFormData(buildCombinedPrompt(prompt, options.systemInstruction), upload);
    const headers = isChatRequest
        ? buildServiceHeaders(serviceConfig, { 'Content-Type': 'application/json' })
    ...
    const result = isChatRequest
        ? getChatResult(serviceConfig.label, await readResponseText(response))
        : getServiceResult(serviceConfig.label, await response.json());
...
async function executeServiceRequest(serviceKey, prompt, upload = null, options = {}) {
    ...
    for (let index = 0; index < serviceConfigs.length; index += 1) {
        ...
        for (let attempt = 1; attempt <= CUSTOM_AI_MAX_ATTEMPTS_PER_MODE; attempt += 1) {
            try {
                const result = await executeSingleServiceRequest(serviceConfig, prompt, upload, options);
...
function createServiceGenerators(serviceKey) {
    ...
    async function generateContent(prompt, systemInstruction = '') {
        ...
        const { result, serviceConfig: resolved, attempt } = await executeServiceRequest(serviceKey, prompt, null, { systemInstruction });
    ...
    async function generateVisionContent(prompt, images = []) {
        ...
        const { result, serviceConfig: resolved, attempt } = await executeServiceRequest(serviceKey, prompt, upload);
