...
export const INFERENCE_CONFIG_ERROR = 'Inference AI service is not configured. Set INFERENCE_BASE_URL and INFERENCE_APP_TOKEN.';
...
function getServiceConfig(serviceKey) {
    ...
    const token = profile.appToken();
    if (!baseUrl || !token) return null;
    return {
        url: `${baseUrl}${profile.publicPath()}`,
...
async function executeSingleServiceRequest(serviceConfig, prompt, upload = null) {
    ...
    try {
        ...
        const networkError = new Error(`${serviceConfig.label} unreachable (${causeCode}): ${causeMessage}`);
...
function createServiceGenerators(serviceKey) {
    ...
    return { generateContent, generateVisionContent, getAvailability };
}
