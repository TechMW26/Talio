...
export async function generatePollinationsImage(prompt, signal) {
  ...
  const response = await fetch('https://gen.pollinations.ai/v1/images/generations', {
    ...
    body: JSON.stringify({ model, prompt, n: 1, size: '1024x1024', quality: 'high', response_format: 'b64_json' }),
  })
  if (!response.ok) {
  ...
  const buffer = await sharp(Buffer.from(encoded, 'base64'), { limitInputPixels: 16777216 }).resize(1024, 1024, { fit: 'inside', withoutEnlargement: true }).png().toBuffer()
