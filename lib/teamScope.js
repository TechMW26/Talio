...
/**
 * Build the fallback scope used when an employee has no reporting chain yet.
 * This keeps new joiners' requests actionable by HR until onboarding assigns
 * their team lead, reporting manager, or executive parent.
 */
function buildUnassignedApprovalFilter() {
  return {
    $and: REPORT_FIELDS.map((field) => ({
      $or: [
        { [field]: { $exists: false } },
        { [field]: null },
      ],
...
function hasReportingChain(employee) {
  if (!employee) return false
  return REPORT_FIELDS.some((field) => Boolean(employee[field]))
}
