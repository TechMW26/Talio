...
async function getSharedTenantRootConnection(seedDatabaseName) {
  // A disconnected driver reconnects itself. Only an explicit close retires
  // the pool; replacing it during a failover leaks sockets and model handles.
  if (sharedTenantRootState.connection) {
  ...
  const rootPromise = (async () => {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      ...
      try {
        ...
        openingConnection = mongoose.createConnection(uri, CONNECTION_OPTIONS);
        ...
        if (!sharedTenantRootState.listenersAttached) {
          ...
          connection.on('close', () => {
            if (sharedTenantRootState.connection === connection) clearSharedTenantState();
          });
        }
...
export async function getTenantConnection(databaseName) {
  ...
  assertDatabaseName(databaseName, { tenant: true });
  ...
  try {
    ...
    if (pendingConnections.get(databaseName) === connectionPromise) {
