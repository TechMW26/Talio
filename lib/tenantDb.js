...
const maxPoolSize = positiveInt(
  process.env.TENANT_DB_MAX_POOL_SIZE,
  isProduction ? 20 : 10
);
const minPoolSize = Math.min(
  positiveInt(process.env.TENANT_DB_MIN_POOL_SIZE, 1),
  maxPoolSize
);

const CONNECTION_OPTIONS = {
  bufferCommands: true,
  // Each tenant has its own connection pool, so conservative defaults prevent
  // total sockets from growing as O(tenants × 50). Both values are tunable.
  maxPoolSize,
  minPoolSize,
  socketTimeoutMS: isProduction ? 90000 : 45000, // Longer timeout for production
  connectTimeoutMS: isProduction ? 30000 : 10000, // More forgiving in production
  serverSelectionTimeoutMS: isProduction ? 30000 : 10000, // More forgiving in production
  wtimeoutMS: 30000,            // 30 seconds write timeout
  family: 4,                    // Force IPv4 to avoid DNS issues
  retryWrites: true,
  retryReads: true,
  maxIdleTimeMS: isProduction ? 120000 : 60000, // 2 minutes idle in production
