...
// Detects only clear, model-issued refusal responses. We must NOT flag long
// legitimate outputs (mindmaps, whiteboard summaries, JSON payloads, etc.) just
// because they contain workplace terms like "safety", "harmful", "blocked", or
// "not allowed". To avoid false positives we require:
//   1. A short response (refusals are typically a sentence or two), AND
//   2. A refusal-style phrase appearing at the very beginning.
// Anything containing structured output (JSON braces, code fences, lists) is
// treated as a real answer, never a refusal.
function isContentPolicyError(text) {
    ...
    // Structured / long-form responses are real answers, not refusals.
    ...
    for (let index = 0; index < customAIConfigs.length; index += 1) {
        ...
        for (let attempt = 1; attempt <= CUSTOM_AI_MAX_ATTEMPTS_PER_MODE; attempt += 1) {
            try {
                ...
                // Fall back to the alternate mode for any error type. Even
                // content-policy refusals from one mode often succeed on the
                // other (different model temperature/safety tuning), and a
                // hard short-circuit causes user-visible failures for normal
                // requests like mindmap generation.
