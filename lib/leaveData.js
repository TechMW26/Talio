...
/** Calendar-day proration, inclusive of joining day, rounded to two decimals. */
export function prorateAnnualLeave(annualDays, joiningDate, year) {
  const entitlement = Math.max(0, toFiniteNumber(annualDays))
  if (!Number.isInteger(Number(year)) || Number(year) < 1900 || Number(year) > 9998) throw new Error('Invalid leave year')
  const joined = joiningDate ? new Date(joiningDate) : null
  if (!joined || Number.isNaN(joined.getTime())) return entitlement
  const start = Date.UTC(year, 0, 1)
  const end = Date.UTC(Number(year) + 1, 0, 1)
  const joinedDay = Date.UTC(joined.getUTCFullYear(), joined.getUTCMonth(), joined.getUTCDate())
  const employedDays = Math.max(0, (end - Math.max(start, joinedDay)) / DAY_IN_MS)
  return Math.round(entitlement * employedDays / ((end - start) / DAY_IN_MS) * 100) / 100
}
